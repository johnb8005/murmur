...
A small, members-only Twitter-like app for sharing links: passkey accounts with unique usernames, one timeline, likes, comments, Web Share, PWA with a share target. A post is called a *murmur* in the UI (`POST_NOUN`); the API and code keep `posts`. Murmurs carry tags (labels: `#hashtags` in the text plus the composer's tag field, `post_tags` table, `/t/:tag` page), may carry one picture (`posts.image_*`, bytes in R2 under `images/<id>` or the `images` table without R2, served at `/images/<id>` with the same viewer rules; `src/image.ts` shrinks it client-side; `public/share-target.js` runs in the service worker so the Android share sheet can hand a picture to `/share`) and may be private (`posts.private`: only the author sees them; every query goes through `loadPosts` / `visiblePostOwnerId` with a viewer). Everything but sign-in, sign-up and device links needs an account. Bun server + Turso + R2, shipped as a container image on GHCR (`.github/workflows/publish.yml` builds the `Dockerfile` for amd64 and arm64 on native runners and merges them into one manifest, on pushes to `main` and `v*` tags; nothing deploys from the repo, the host pulls `ghcr.io/johnb8005/murmur` and sets the env from `.env.example`). Johan Boissard's identity page (the `personal-website` repo) links here.
...
- `bun test` — unit tests (`test/*.test.ts`: the health probe's failure paths with a mocked database; also run by `test.yml`)
...
- `server/index.ts` — `Bun.serve`: built app, `/feed.xml`, `/feed.json`, `/previews/<hash>.jpg` and `/images/<post id>` (session cookie or `?token=<feed token>`, else 401; pictures also need the post to be visible), `POST /share` (no-service-worker fallback: redirect to the composer), `/api/health` (database round trip, 503 when it fails; the Dockerfile's `HEALTHCHECK` probes it), oRPC at `/rpc` (protocol) and `/api` (OpenAPI style). Every page gets the same generic Open Graph card (the `<!-- og -->` placeholder in `index.html`, image `public/og.jpg` from `scripts/og-card.ts`; a `/p/:id` URL only says a murmur was shared): post content must not unfurl, and `sharePost` shares the address alone
